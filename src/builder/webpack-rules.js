...
module.exports = function () {
                ...
                options: {
                    name: path => {
                        if (! /node_modules|bower_components/.test(path)) {
                            return 'assets/' + Config.fileLoaderDirs.images + '/[name].[ext]?[hash]';
                        }

                        return 'assets/' + Config.fileLoaderDirs.images + '/vendor/' + path
                            .replace(/\\/g, '/')
                            .replace(
    ...
    rules.push({
        ...
        options: {
            name: path => {
                if (! /node_modules|bower_components/.test(path)) {
                    return 'assets/' + Config.fileLoaderDirs.fonts + '/[name].[ext]?[hash]';
                }

                return 'assets/' + Config.fileLoaderDirs.fonts + '/vendor/' + path
                    .replace(/\\/g, '/')
                    .replace(
